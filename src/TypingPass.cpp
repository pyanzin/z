...
void TypingPass::visit(ZAssign* zassign) {
    ...
    if (!dynamic_cast<ZSubscript*>(assignee) && !assignee->getType()->isEqual(*zassign->getRight()->getType()))
        error("Type of the left-hand expression doesn't match the type of right-hand expression",
              assignee->getSourceRange());
...
void TypingPass::visit(ZCall* zcall) {
    ...
    if (zselector) {
        ...
        auto newCall = new ZCall(new ZId(*methodName, zcall->getRef()), *newArgs, zcall->getTypeArgs(),
                                 zcall->getRef());
    ...
    if (calleeArgsCount != callerArgsCount)
        error("Callee requires " + std::to_string(calleeArgsCount) + " arguments, but caller passes " + std::
              to_string(callerArgsCount),
...
void TypingPass::visit(ZBinOp* zbinop) {
    ...
    error("Unable to apply operation " + toString(zbinop->getOp()) + " for "
          + zbinop->getLeft()->getType()->toString() + " and " + zbinop->getRight()->getType()->toString(),
          zbinop->getSourceRange());
}
...
void TypingPass::visit(ZId* zid) {
    ...
    if (definition->symbolType == Field) {
        auto thisSelector = new ZSelector(new ZId(*new std::string("this"), zid->getRef()),
                                          new std::string(zid->getName()), zid->getRef());
...
void TypingPass::visit(ZVarDef* zvardef) {
    ...
    if (alreadyDefined->size() > 0)
        error("Variable with name '" + (*alreadyDefined)[0]->getName() + "' already defined in this scope",
              zvardef->getSourceRange());
